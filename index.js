...
function game(question, options, answer) {
  ...
  console.log(chalk.bgCyanBright.black.bold(`Your current score is ${score} \n`));

  return 0;

}

console.log(chalk.bgMagenta.whiteBright.bold("Welcome to the game, WHAT IS THIS USED FOR ! \n"));

let userName = read.question(chalk.bgMagenta.whiteBright.bold("What is your name ? \n\n"));

console.log(chalk.bgMagenta.whiteBright.bold(`\n Okay ${userName}, before we begin \n`));


console.log(chalk.bgCyanBright.black.bold("Here is the current leaderboard with top 3 high scoring participants, let's see if you can get a new high score ! \n"));
...
for (let item of questionsList) {
  ...
  let reply = read.question(chalk.bgCyanBright.black.bold("Do you want to continue y/n ? \n\n"));
  console.log("----------------------------------------------------------------------------");
  if (reply === "n") {
    break;
  }
